...
from sesame.utils.c2_model_loading import get_name_convert_func
...
def load_checkpoint(
    ...
    log.info("Loading network weights from {}.".format(path_to_checkpoint))
        ...
        if inflation:
                ...
                for item in clear_name_pattern:
                    ...
                    for k in checkpoint["model_state"]:
                        if item in k:
                            ...
                            log.info("renaming: {} -> {}".format(k, k_re))
                        else:
            ...
            if not_load_layers:
                for k in not_load_layers:
                    log.info("Network weights {} not loaded.".format(k))
...
def load_train_checkpoint(cfg, model, optimizer, scaler=None):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        log.info("Load from last checkpoint, {}.".format(last_checkpoint))
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, cfg.NUM_GPUS > 1, optimizer, scaler=scaler
        )
        start_epoch = checkpoint_epoch + 1
    elif cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        log.info("Load from given checkpoint file.")
        checkpoint_epoch = load_checkpoint(

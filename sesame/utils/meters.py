...
from sesame.utils.timer import Timer
...
def get_map(preds, labels):
    ...
    log.info("Getting mAP for {} examples".format(preds.shape[0]))

    preds = preds[:, ~(np.all(labels == 0, axis=0))]  # 清除 labels 不含的某一类，即该批次内该类标签均为零

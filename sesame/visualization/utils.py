...
from sesame.datasets.utils import pack_pathway_output, tensor_normalize
...
def process_cv2_inputs(frames, cfg):
    ...
    Args:
        frames (list of array): list of input images (correspond
                                to one clip) in range [0, 255].
        cfg (CfgNode): configs.
    """
    ...
    inputs = [inp.unsqueeze(0) for inp in inputs] # add batch axis
    return inputs

...
from sesame.visualization.video_visualizer import VideoVisualizer

log = logger.get_logger(__name__)


class AVAVisualizerWithPrecomputedBox(object):
    """
    Visualize action predictions for videos or folder
    of images with precomputed and ground-truth boxes
    in AVA format.
    """
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): configs. Details can be found in
                        'sesame/config/defaults.py'
        """
        self.source = os.fspath(path=cfg.DEMO.INPUT_VIDEO)
        self.fps = None
    ...
    def get_input_clip(self, keyframe_idx):
        ...
        Returns:
            clip (list of tensors): formatted input clips corresponding
    ...
    def get_predictions(self, model):
        """
        Predict and append predict results to each box in
        each keyframe in 'self.pred_boxes' dictionary.
        Args:
            model (torch.nn.Module): video model to evaluate
        """
        ...
        logger.setup_logging(self.cfg.OUTPUT_DIR)

        # print config.
        log.info("Run demo with config:")
        log.info(self.cfg)
    ...
    def draw_video(self):
        """
        Draw predicted and ground-truth (if provided) results
        on the video/folder of images. Write the visualized
        result to a video output file.
        """
    ...
    def __call__(self, model):
        self.get_predictions(model)
        self.draw_video()

    def display(self, frame):
        """
        Either display a single frame (BRG image) to a window or
        write to an output file if output path is provided.
        """
    ...
    def _get_frame_range(self, start_idx, num_frames):
        """
        Return a clip of 'num_frames' frames starting from 'start_idx'. if
        not enough frames from 'start_idx', return the remaining frames from
        'start_idx'.
        Args:
...
def merge_pred_gt_boxes(pred_dict, gt_dict=None):
    ...
    Args:
        ...
        gt_dict (Optional[dict]): a dict which maps from 'frame_idx' to a list
            of 'boxes' and `labels`. Each `box` is a list of 4 box coordinates.
            `labels[i]` is a list of labels for `boxes[i]`. Note that label is -1
            for predicted boxes.
    Returns:
...
def load_boxes_labels(cfg, video_name, fps, img_width, img_height):
    ...
    def process_bboxes_dict(dictionary):
        """
        Replace all `keyframe_sec` in `dictionary` with `keyframe_idx` and merge all
        [`box_coordinate`, `box_labels`] pairs into [`all_boxes_coordinates`, `all_boxes_labels`]
        for each 'keyframe_idx'.
        Args:
            dictionary (dict): a dictionary which maps `frame_sec` to a list of `box`.
                ...
                coordinates of box and 'box_labels` are the corresponding labels for the box.
        Returns:
            new_dict (dict): a dict which maps from `frame_idx` to a list of `boxes`
                and `labels`. Each `box` in `boxes` is a list of 4 box coordinates.
                `labels[i]`
        """
